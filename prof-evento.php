...
$app->get("/prof/evento/create", function() {
...
$app->post("/prof/evento/create", function() {
	...
	if(!isset($_POST['tem_regras']) ||$_POST['tem_regras'] == ""){
		echo "<script>alert('Informe se o evento tem regras, se tiver, você terá que inserir um arquivo, em pdf, com as tais!');";
	  	echo "javascript:history.go(-1)</script>";
	   	exit;
	}

	if($_POST["tem_regras"] == 1){
		...
		$renomear_arquivo_regras = date('Ymd').'_'.$_POST['iduser'].'_'.md5(time()).'.pdf';

		$caminho_upload_regras = 'res/admin/evento/regras/';	
		
	}else{

		$renomear_arquivo_regras = '';
	}
	...
	if(!isset($_POST['tem_imagem']) ||$_POST['tem_imagem'] == ""){
		echo "<script>alert('Informe se o evento terá uma imagem para a página inicial do site!');";
	  	echo "javascript:history.go(-1)</script>";
	   	exit;
	}

	*/
	if($_POST["tem_imagem"] == 1){
		...
		$renomear_arquivo_imagem = date('Ymd').'_'.$_POST['iduser'].'_'.md5(time()).'.png';
		$caminho_upload_imagem = 'res/admin/evento/img/';

	}else{

		$renomear_arquivo_imagem = '';
	}
	...
	$imagem_arquivo_evento = $renomear_arquivo_imagem; 
	$pdf_regras_evento = $renomear_arquivo_regras; 
	...
	*/
	...
	var_dump($_POST['tem_imagem'].' - '.$_POST['tem_regras']);
	exit();

	if($_POST["tem_imagem"] == 1){
		
		move_uploaded_file($arquivo['imagem_arquivo_evento']['tmp_name'], $caminho_upload_imagem . $renomear_arquivo_imagem);
	}

	if($_POST["tem_regras"] == 1){
		
		move_uploaded_file($arquivo['pdf_regras_evento']['tmp_name'], $caminho_upload_regras . $renomear_arquivo_regras);
	}
...
$app->get("/prof/evento/regras-update/:id_evento", function($id_evento) {
...
$app->post("/prof/evento/regras-upload", function() {
...
$app->get("/prof/evento/regras-delete/:id_evento", function($id_evento) {
...
$app->get("/prof/evento/imagem-update/:id_evento", function($id_evento) {
...
$app->post("/prof/evento/imagem-upload", function() {
...
$app->get("/prof/eventos-por-temporada/:idtemporada", function($idtemporada) {
...
$app->get("/prof/eventos-por-temporada/:idtemporada/local/:idlocal", function($idtemporada, $idlocal) {
...
$app->get("/prof/atualiza/evento/:id_evento/:idtemporada/:desctemporada/:status", function($id_evento, $idtemporada, $desctemporada, $status) {
...
$app->get("/prof/insc-evento-temporada/:id_evento/:idtemporada/user/:iduser", function($id_evento, $idtemporada, $iduser) {
